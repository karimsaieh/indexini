...
try:
    ...
    result_df = spark_utils \
        .join_df(result_df, kmeans_df, "url",
                 ["url", "file_name", "timestamp", "uuid", "pre_processed_text", "summary", "most_common", "thumbnail", "content_type", "bisecting_kmeans_prediction", "content"],
                 ["kmeans_prediction"])
    result_df = spark_utils \
        .join_df(result_df, lda_with_count_vectorizer_df, "url",
                 ["url", "file_name", "timestamp", "uuid", "pre_processed_text", "summary", "most_common", "thumbnail", "content_type", "kmeans_prediction", "bisecting_kmeans_prediction", "content"],
                 ["lda_topics"])

    if os.environ["pfe_env"] == "dev":
        result_df.show(n=200)
        listr = result_df.take(1000)
        for l in listr:
            print(l["file_name"], l["lda_topics"])
